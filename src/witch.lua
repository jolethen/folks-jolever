-- src/witch.lua
-- Witch NPC Exchange Interface for Techblox
...
-- 2. FORMSPEC GUI LOBBY
local function show_witch_formspec(player_name)
    local formspec = 
        "size[9.5,7.0]" ..
        "real_coordinates[true]" ..
        "background[0,0;9.5,7.0;#161224;true]" .. -- Slightly darker purple/witchy tint
...
-- 5. REGISTER THE WITCH NPC ENTITY/NODE
-- Using a standard node layout so you can easily place her inside town structures or spawn frames
core.register_node("folks:witch_npc", {
    description = "Witch Altar NPC",
    drawtype = "mesh",
    mesh = "character.b3d", -- Leverages standard Luanti character body meshes
    tiles = {"character.png"}, -- Put your custom skin name here if you have a special witch texture
    paramtype = "light",
    paramtype2 = "facedir",
    walkable = true,
    selection_box = {
        type = "fixed",
        fixed = {-0.3, -0.5, -0.3, 0.3, 1.3, 0.3}
    },
    collision_box = {
        type = "fixed",
        fixed = {-0.3, -0.5, -0.3, 0.3, 1.3, 0.3}
    },
    groups = {cracky = 3, oddities = 1, cant_to_protect = 1},

    on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
        if not clicker or not clicker:is_player() then return itemstack end
        show_witch_formspec(clicker:get_player_name())
        return itemstack
    end,
})
