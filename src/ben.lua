...
local BEN_BUYS = {
	item1 = {
		item = "forgotten_monsters:buried_bone",
		payout = 5
	},
	payout_item = "currency:minegeld"
}

local function show_ben_gui(player_name)
	local formspec = 
		"size[8.5,4.5]" ..
		"background[0,0;8.5,4.5;techblox_terminal_bg.png;true]" .. 
		"box[0,0;8.5,0.1;#a832a4]" ..
		"label[0.5,0.5;" .. core.colorize("#e066ff", "BEN - EXCAVATION DEALER") .. "]" ..
		"box[0.5,0.9;7.5,0.02;#ffffff15]" ..
		
		-- Row 1: Buried Bone
		"box[0.5,1.3;7.5,1.3;#ffffff03]" ..
		"item_image[0.8,1.4;1,1;" .. BEN_BUYS.item1.item .. "]" ..
		"label[2.0,1.5;" .. core.colorize("#ffffff", "Sell Buried Bone") .. "]" ..
		"label[2.0,2.0;" .. core.colorize("#00ff00", "Payout: " .. BEN_BUYS.item1.payout .. " Minegeld each") .. "]" ..
		"button[5.6,1.5;2,0.6;sell_item1;Sell Stack]" ..
		
		"button_exit[3.2,3.5;2,0.6;quit;Close]"

	core.show_formspec(player_name, "folks:merchant_ben", formspec)
end

local function handle_ben_sale(player, player_name, inv, target_config)
	local check_stack = ItemStack(target_config.item)
	if check_stack:is_empty() or not inv:contains_item("main", check_stack) then
		core.chat_send_player(player_name, core.colorize("#ff3333", "[Ben]: You don't have that item on you!"))
		return
	end
	...
	if total_sold > 0 then
		...
		local payout_stack = ItemStack(BEN_BUYS.payout_item .. " " .. total_payout)
		...
		core.chat_send_player(player_name, core.colorize("#00ff00", "[Ben]: Great finds. Paid out " .. total_payout .. " Minegeld."))
		show_ben_gui(player_name)
	end
end

core.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "folks:merchant_ben" then return false end
	...
	if fields.sell_item1 then
		handle_ben_sale(player, player_name, inv, BEN_BUYS.item1)
		return true
	end
end)
...
return {
	on_interact = function(player, npc_self)
		show_ben_gui(player:get_player_name())
	end
}
